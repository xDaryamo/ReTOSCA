...
class AWSVPCSecurityGroupEgressRuleMapper(SingleResourceMapper):
    ...
    def can_map(self, resource_type: str, resource_data: dict[str, Any]) -> bool:
        """
        This mapper is specific to the 'aws_vpc_security_group_egress_rule' type.
        """
        return resource_type == "aws_vpc_security_group_egress_rule"
    ...
    def map_resource(
        ...
        """
        Map aws_vpc_security_group_egress_rule by adding metadata to the
        ...
        if not context:
            logger.warning(
                f"No context provided for egress rule '{resource_name}'. Skipping."
            )
            return
    ...
    def _add_egress_rule_to_node(self, sg_node, rule_metadata: dict[str, Any]) -> None:
        """
        Add the egress rule metadata to the security group node.
        ...
        # Initialize egress_rules list if it doesn't exist
        if "egress_rules" not in current_metadata:
            current_metadata["egress_rules"] = []

        # Add the new rule
        current_metadata["egress_rules"].append(rule_metadata)
