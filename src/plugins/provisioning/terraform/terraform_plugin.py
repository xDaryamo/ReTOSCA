...
# Import base classes
from src.core.common.base_phase_plugin import BasePhasePlugin
...
class TerraformProvisioningPlugin(BasePhasePlugin):
    """
    Phase plugin for Terraform infrastructure provisioning.

    This plugin handles Terraform configuration files and maps them to TOSCA
    infrastructure provisioning nodes. It connects the TerraformParser with
    the TerraformMapper and all registered AWS resource mappers.
    """
    ...
    def get_plugin_info(self) -> dict:
        ...
        """
        return {
            "name": "TerraformProvisioningPlugin",
            "phase": "provisioning",
            "technology": "terraform",
            "supported_extensions": self._parser.get_supported_extensions(),
            "registered_mappers": list(self._mapper.get_registered_mappers().keys()),
            "description": (
                "Maps Terraform infrastructure configurations "
                "to TOSCA provisioning nodes"
            ),
        }
