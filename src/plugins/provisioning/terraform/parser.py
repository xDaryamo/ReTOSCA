...
class TerraformParser(BaseSourceFileParser):
    ...
    def _run_command(
        ...
        # Check if we're in quiet mode and should suppress output
        import os

        quiet_mode = os.environ.get("TOSCA_QUIET_MODE") == "1"
        if quiet_mode and not capture_output:
            # Force capture output in quiet mode to prevent console spam
            capture_output = True
